...
function ModalNuevoExterno({ onClose, onCreado, apiKey }) {
  ...
  const [creado,       setCreado]       = useState(null); // { username, password, linkPago }
  ...
  const username = `${nombre.toLowerCase().replace(/\s+/g, "").slice(0, 6)}${apellido.toLowerCase().replace(/\s+/g, "").slice(0, 4)}`.replace(/[^a-z0-9]/g, "") || "externo";
  const centroId = username;

  async function handleGuardar() {
    setError(null);
    if (!nombre || !email || !rut) { setError("Nombre, RUT y email son obligatorios"); return; }
    setSaving(true);
    ...
    try {
      ...
      if (!profRes.ok) {
        const e = await profRes.json();
        throw new Error(e.detail || "Error creando profesional");
      }

      // 2. Crear usuario con scope externo
      ...
      if (!userRes.ok) {
        ...
        throw new Error(e.detail || "Error creando usuario");
      }
      ...
      const susRes = await fetch(`${API_URL}/api/superadmin/suscripciones`, {
        ...
        body: JSON.stringify({
          centro_id:        centroId,
          nombre_centro:    `${nombre} ${apellido}`.trim(),
          plan,
          roles:            {},
      ...
      try {
        await fetch(`${API_URL}/api/superadmin/externos/${centroId}/enviar-credenciales`, {
          method: "POST", headers,
          body: JSON.stringify({
            email_contacto: email,
            nombre:         `${nombre} ${apellido}`.trim(),
        ...
        console.warn("Error enviando credenciales:", e);
      }
      ...
      <div style={styles.modal}>
        ...
        {creado.linkPago && (
          <div style={{ background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 8,
            padding: "10px 12px", fontSize: 12, wordBreak: "break-all", marginBottom: 12 }}>
            {creado.linkPago}
          </div>
        )}
        <div style={{ display: "flex", gap: 8 }}>
          {creado.linkPago && (
            <button style={styles.btnPrimary}
              onClick={() => navigator.clipboard.writeText(creado.linkPago).catch(() => {})}>
              Copiar link
            </button>
          )}
  ...
  return (
    <div style={styles.overlay}>
      <div style={styles.modal}>
        ...
        <Field label="RUT *">
          <input style={styles.input} value={rut} placeholder="12345678-9"
  ...
  return (
    <div className="page">
      ...
      {loading ? <p style={{ color: "#94a3b8" }}>Cargando…</p> :
        lista.length === 0
          ? <p style={{ color: "#94a3b8", textAlign: "center", padding: 40 }}>Sin externos registrados</p>
          : lista.map(s => {
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)",
          ...
          <div style={{ background: "#fff", borderRadius: 16, padding: 24, maxWidth: 360, width: "100%" }}>
            ...
            <p style={{ fontSize: 13, color: "#64748b", marginBottom: 20 }}>
              Se eliminará la suscripción de <strong>{confirmBorrar.nombre_centro}</strong>. Esta acción no se puede deshacer.
            </p>
            <div style={{ display: "flex", gap: 8 }}>
